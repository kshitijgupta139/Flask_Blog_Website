...
class Posts(db.Model):     #will define the tables of sql
    ...
    img_file = db.Column(db.String(50), unique=False, nullable=True)
...
def contact():
    if (request.method == 'POST'):
        ...
        entry = Contacts(name=name,  email=email, phno=phone, msg=message, date=datetime.now())
...
def delete(sno):
    if 'user' in session and session['user'] == params['admin_user']:
        ...
        db.session.commit()
        flash("Successfully deleted...","primary")
...
@app.route("/logout")
def logout():
    session.pop('user')  #to kill user session
    return redirect('/dashboard')
    ...
    if 'user' in session and session['user'] == params['admin_user']:
        if request.method == 'POST':
            ...
            if sno == '0':
                ...
                post.date=date
                db.session.commit()
                flash("Successfully edited..","primary")
